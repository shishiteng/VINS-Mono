...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_lidar_path = n.advertise<nav_msgs::Path>("lidar_path", 1000);
    pub_lidar_pose = n.advertise<geometry_msgs::PoseStamped>("lidar_pose", 1000);
...
void pubLatestOdometry(const Estimator &estimator, const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header)
{
...
void printStatistics(const Estimator &estimator, double t)
{
    if (estimator.solver_flag != Estimator::SolverFlag::NON_LINEAR)
        return;
    //printf("position: %f, %f, %f distance: %.1f \r", estimator.Ps[WINDOW_SIZE].x(), estimator.Ps[WINDOW_SIZE].y(), estimator.Ps[WINDOW_SIZE].z(), distance_);
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        lidar_pose.header = header;
        lidar_pose.header.frame_id = "world";
        lidar_pose.pose.position.x = Plw.x();
        lidar_pose.pose.position.y = Plw.y();
        lidar_pose.pose.position.z = Plw.z();
        lidar_pose.pose.orientation.x = Rlw.x();
        lidar_pose.pose.orientation.y = Rlw.y();
        lidar_pose.pose.orientation.z = Rlw.z();
        lidar_pose.pose.orientation.w = Rlw.w();
